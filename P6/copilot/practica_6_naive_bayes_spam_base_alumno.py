...
def load_dataset(csv_path: str | Path) -> pd.DataFrame:
    ...
    # Crear etiqueta numérica para clasificación binaria
...
def prepare_data(df: pd.DataFrame):
    ...
    # Dividir en train y test conservando proporciones de clase
    ...
    # Vectorizador Bag of Words con preprocesado personalizado
    vectorizer = CountVectorizer(preprocessor=clean_text)

    # Matriz documento-término en train y test
...
def evaluate_model(model, X_test_dtm, y_test, output_dir: str | Path | None = None):
    ...
    plt.title("Matriz de confusión - Naive Bayes Spam")
    plt.tight_layout()
...
def get_top_spam_words(model, vectorizer, top_n: int = 5) -> pd.DataFrame:
    ...
    # Diferencia de asociación con spam frente a ham
...
def classify_custom_messages(messages: list[str], model, vectorizer) -> pd.DataFrame:
    ...
    results = pd.DataFrame(
        {
            "message": messages,
            "predicted_label": np.where(predicted_class == 1, "spam", "ham"),
            "P(ham)": predicted_proba[:, 0],
            "P(spam)": predicted_proba[:, 1],
        }
    )
    results["P(ham)"] = results["P(ham)"].round(4)
    results["P(spam)"] = results["P(spam)"].round(4)
    return results
...
def main() -> None:
    ...
    print(f"P(ham):  {class_priors[0]:.4f}")
    print(f"P(spam): {class_priors[1]:.4f}")
